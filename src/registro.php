    ...
    if (empty($_POST['nombre']) || empty($_POST['apellido']) || empty($_POST['correo']) || empty($_POST['contrasena']) || empty($_POST['repetir_contrasena']) || empty($_POST['numero_documento'])) {
        ...
        if (mysqli_num_rows($result_verificacion_correo) > 0) {
            ...
            if(isset($_FILES['foto_perfil'])) {
                ...
                $temp_location = $_FILES['foto_perfil']['tmp_name'];
                $target_location = "fotosperfiles/" . $foto_perfil_nombre;
                
                if(move_uploaded_file($temp_location, $target_location)) {
                    // Archivo subido exitosamente
                    $foto_perfil = $target_location;
                } else {
                    // Error al subir el archivo
                    echo json_encode(array("success" => false, "mensaje" => "Error al subir la foto de perfil. Por favor, inténtelo de nuevo."));
                    exit();
                }
            } else {
                echo json_encode(array("success" => false, "mensaje" => "Es necesario proporcionar una foto de perfil."));
                exit();
            }
