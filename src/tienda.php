...
if(isset($_POST['add_to_cart'])) {
    ...
    $sql = "INSERT INTO carrito (id_producto, id_usuario, fecha) VALUES ('$id_producto', '$id_usuario', '$fecha')";
...
<script>
    ...
    function addToCart(producto_id) {
        // Enviar solicitud AJAX para agregar el producto al carrito
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "tienda.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4 && xhr.status === 200) {
                // Actualizar interfaz de usuario u ofrecer retroalimentación
                console.log(xhr.responseText);
            }
        };
        xhr.send("add_to_cart=true&producto_id=" + producto_id);
    }
</script>
